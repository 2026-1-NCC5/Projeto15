...
// ============================================================
// TOKENS DE DESIGN (idênticos ao AlunoPainel)
// ============================================================
const T = {
  bg:          '#0e1612',
  bgPanel:     '#141f18',
...
const AdminPainel = () => {
  ...
  useEffect(() => {
    ...
    // Verifica cargo no Firestore para garantir que é admin
    const verificarAdmin = async () => {
      try {
        const snap = await getDocs(query(collection(db, 'users'), where('email', '==', userData.email)));
        if (snap.empty) { setAuthError('Usuário não encontrado.'); setLoading(false); return; }

        const docData = snap.docs[0].data();
        const cargo = (docData.cargo || docData.role || docData.tipo || '').toLowerCase();

        if (cargo !== 'admin' && cargo !== 'administrador') {
          setAuthError('Acesso restrito. Você não tem permissão de administrador.');
          setLoading(false);
          return;
        }

        // Atualiza localStorage com cargo confirmado
        const userCompleto = { ...userData, cargo, uid: snap.docs[0].id };
  ...
  if (authError) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: T.bg, fontFamily: T.fontBody }}>
      <div style={{ textAlign: 'center', padding: 48, background: T.bgCard, borderRadius: T.radius, border: `1px solid ${T.roseDim}`, maxWidth: 400 }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>🔒</div>
        <h2 style={{ fontFamily: T.fontDisplay, color: T.textPrimary, margin: '0 0 12px' }}>Acesso negado</h2>
        <p style={{ color: T.textSecond, fontSize: 14, margin: '0 0 24px' }}>{authError}</p>
      ...
      <main style={{ flex: 1, padding: '36px 40px', overflowY: 'auto', minWidth: 0 }}>
        ...
        {abaAtiva === 'visaogeral' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {/* Últimas doações (visão geral, sem filtro) */}
        ...
        {abaAtiva === 'metas' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button className="ap-btn-primary" onClick={() => abrirModalMeta()}>
                + Nova meta
              </button>
            </div>
        ...
        {abaAtiva === 'usuarios' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button className="ap-btn-primary" onClick={() => abrirModalUsuario()}>
                + Novo usuário
              </button>
            </div>
